...
class UploadService:
    ...
    def _process_file(self, file_path: Path, job_id: str) -> Dict[str, Any]:
        """Run full processing pipeline (OCR → Reconstruction → PDF → Summary)."""
        try:
            ...
            if not pages:
                raise ValueError("No text found in OCR output")

            # Step 2: Create chunks for LLM processing
            logger.info("🧩 Creating text chunks for reconstruction...")
            combined_text = " ".join([page.get("text", "") for page in pages if page.get("text")])
            if not combined_text.strip():
                raise ValueError("No readable text found after OCR")
            ...
            pdf_result = self._generate_pdf(chunks_out, job_id)
    ...
    # ----------------------------------------------------------------
    def _generate_pdf(self, chunks: List[Dict[str, Any]], job_id: str) -> Dict[str, Any]:
        """Generate PDF from reconstructed content."""
        try:
            if not chunks:
            ...
            content = [
                {
                    "title": " ".join(chunk.get("heading_buffer", [])) or f"Section {i+1}",
                    "content": "\n".join(chunk.get("content_buffer", [])),
                    "level": 1,
                }
            ...
            result = self.pdf_agent.generate_pdf(
                content=content, output_path=str(output_path), title="Reconstructed Document"
            )

            if not result.get("success"):
                raise Exception(result.get("error", "PDF generation failed"))

            return {"success": True, "file_path": str(output_path), "page_count": len(chunks)}
